...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //добавляю свой слушатель селектора
  document
      .querySelector("#getemail")
...
function load_mailbox(mailbox, message = "") {
  ...
  + mailbox.slice(1)
    ...
    //создаю дату и причесываю
    let date_filter = document.getElementById("date_filter").value;
    let selected_date = new Date(date_filter);
  ...
  fetch(`/emails/${mailbox}`)
    ...
    .then((emails) => {
      emails.forEach((item) => {
        let item_date = new Date(item.timestamp);
        item_date.setHours(0,0,0,0)
        if (item_date.toString() == selected_date.toString() || date_filter == "") {
...
function build_emails(item, parent_element, mailbox) {
  ...
  let letter_style = '';
  if (mailbox == 'inbox'){
    letter_style = "btn-sm btn-outline-primary";
  } else if (mailbox == 'sent'){
    letter_style = "btn-sm btn-outline-success"
  } else if (mailbox == 'archive'){
    letter_style = "btn-sm btn-outline-secondary"
  }

  if (item["read"]) {
    /*parent_element.style.backgroundColor = "grey";
    date.style.color = "black";*/
    parent_element.className = letter_style;
  } else {
  ...
  //parent_element.className = 'btn-outline-primary';
}
